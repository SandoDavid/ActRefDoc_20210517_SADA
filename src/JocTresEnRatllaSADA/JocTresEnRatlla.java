package JocTresEnRatllaSADA;
import java.awt.*;
import java.awt.event.*;
import javax.swing.*;

@SuppressWarnings("serial")
public class JocTresEnRatlla extends JFrame {
...
private boolean diagonalSecundaria;
         ...
         public void mouseClicked(MouseEvent e) {  // Gestor del clic del ratolí
            ...
            if (estatActual == EstatDelJoc.EN_JOC) {
               if (filaSeleccionada >= 0 && filaSeleccionada < 3 && coluSeleccionada >= 0
                  ...
                  actualitzaElJoc(jugadorActual, filaSeleccionada, coluSeleccionada); // actualitza l'estat del joc
                  // Canvia de jugador
   ...
   public void actualitzaElJoc(Cella laTirada, int filaSeleccionada, int coluSeleccionada) {
   ...
   public boolean hiHaJuanyador(Cella laTirada, int filaSeleccionada, int coluSeleccionada) {
      ...
      setDiagonalPrincipal(false);
      setDiagonalSecundaria(false);
      ...
      public void paintComponent(Graphics g) {  // invocar mitjançant repaint()
         ...
         if (estatActual == EstatDelJoc.EN_JOC) {
            ...
            if (jugadorActual == Cella.CREU) {
               String textTornDeLesXs = "Torn de les X's";
			barraDEstat.setText(textTornDeLesXs);
            } else {
               String textTornDeLesOs = "Torn de les O's";
			barraDEstat.setText(textTornDeLesOs);
            }
         } else if (estatActual == EstatDelJoc.EMPAT) {
            barraDEstat.setBackground(Color.MAGENTA);
            barraDEstat.setForeground(Color.RED);
            barraDEstat.setText("Hi ha empat! Pitja per tornar a jugar!");
         } else if (estatActual == EstatDelJoc.GUANYA_CREU) {
            barraDEstat.setForeground(Color.RED);
            barraDEstat.setText("Guanyen les 'X'! Pitja per tornar a jugar!");
         } else if (estatActual == EstatDelJoc.GUANYA_CERCLE) {
            barraDEstat.setForeground(Color.RED);
            barraDEstat.setText("Guanyen les 'O'! Pitja per tornar a jugar!");
         }
      }
   }
   ...
   public static void main(String[] args) {
      ...
      SwingUtilities.invokeLater(new Runnable() {
         public void run() {
            new JocTresEnRatlla(); // Crida al costructor
         }
      });
   }

public boolean isDiagonalPrincipal() {
...
public boolean isDiagonalSecundaria() {
